...
int32_t blueftl_user_ftl_create (struct ssd_params_t* ptr_ssd_params)
{
	...
	_ftl_base = ftl_base_dftl_mapping;
	...
	uint8_t* ptr_buffer)
{
...
#if(TRACE==TRACE_REPLAY)
	if(_ptr_vdevice == NULL)
	{
	...
	lpa_begin = lba_begin / (uint32_t) _ptr_vdevice->page_main_size;
	lpa_end = lpa_begin + (lba_end / (uint32_t) _ptr_vdevice->page_main_size);
		...
		case NETLINK_WRITE:
			for (lpa_curr = lpa_begin; lpa_curr < lpa_end; lpa_curr++) {
				...
				if (is_merge_needed == 0) {
					((struct ftl_page_mapping_context_t*)_ptr_ftl_context->ptr_mapping)->nr_free_pages--;
